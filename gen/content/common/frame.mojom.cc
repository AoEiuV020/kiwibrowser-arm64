...
bool Frame_ExtractSmartClipData_ForwardToCallback::Accept(
  ...
  if (!input_data_view.ReadText(&p_text))
    success = false;
  if (!input_data_view.ReadHtml(&p_html))
    success = false;
  if (!input_data_view.ReadClipRect(&p_clip_rect))
    success = false;
  if (!success) {
    ReportValidationErrorForMessage(
        message,
        mojo::internal::VALIDATION_ERROR_DESERIALIZATION_FAILED,
        "Frame::ExtractSmartClipData response deserializer");
    return false;
  }
  if (!callback_.is_null())
    std::move(callback_).Run(
...
#if BUILDFLAG(MOJO_TRACE_ENABLED)
  TRACE_EVENT1("mojom", "(Impl)content::mojom::Frame::ExtractSmartClipData",
               "message", message->name());
#endif
      break;
    }
  ...
  TRACE_EVENT1("mojom", "(Impl)content::mojom::Frame::ExtractSmartClipData",
      ...
      if (!success) {
        ReportValidationErrorForMessage(
            message,
            mojo::internal::VALIDATION_ERROR_DESERIALIZATION_FAILED,
            "Frame::ExtractSmartClipData deserializer");
        return false;
      }
      Frame::ExtractSmartClipDataCallback callback =
