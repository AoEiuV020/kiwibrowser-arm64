...
SessionData::SessionData(
    const std::vector<uint8_t>& key_set_id_in,
...
class MediaDrmStorage_Initialize_ProxyToResponder {
 ...
 private:
  ...
  void Run(
      const base::UnguessableToken& in_origin_id);
...
bool MediaDrmStorage_Initialize_ForwardToCallback::Accept(
  ...
  bool success = true;
  base::UnguessableToken p_origin_id{};
  ...
  proxy_->Initialize(
      base::BindOnce(
          [](base::RunLoop* loop,
             base::UnguessableToken* out_origin_id
,
             const base::UnguessableToken& origin_id) {*out_origin_id = std::move(origin_id);
            loop->Quit();
          },
          &loop,
          out_origin_id));
