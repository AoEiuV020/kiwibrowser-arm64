...
#if BUILDFLAG(MOJO_TRACE_ENABLED)
  TRACE_EVENT1("mojom", "(Impl)viz::mojom::GpuHost::DisableGpuCompositing",
               "message", message->name());
#endif
      mojo::internal::MessageDispatchContext context(message);
      ...
      if (!success) {
        ReportValidationErrorForMessage(
            message,
            mojo::internal::VALIDATION_ERROR_DESERIALIZATION_FAILED,
            "GpuHost::DisableGpuCompositing deserializer");
        return false;
      }
      // A null |impl| means no implementation was bound.
      assert(impl);
...
bool GpuHostRequestValidator::Accept(mojo::Message* message) {
  ...
  switch (message->header()->name) {
    ...
    case internal::kGpuHost_DisableGpuCompositing_Name: {
      ...
      if (!mojo::internal::ValidateMessagePayload<
               internal::GpuHost_DisableGpuCompositing_Params_Data>(
                  message, &validation_context)) {
        return false;
      }
      return true;
