...
const char OriginTrials::kEventTimingTrialName[] = "EventTiming";
...
bool OriginTrials::eventTimingEnabled(const ExecutionContext* executionContext) {
  if (RuntimeEnabledFeatures::EventTimingEnabledByRuntimeFlag())
    return true;

  const OriginTrialContext* context = OriginTrialContext::From(executionContext);
  if (!context) return false;
  if (context->IsTrialEnabled(kEventTimingTrialName))
    return true;
  return false;
}
