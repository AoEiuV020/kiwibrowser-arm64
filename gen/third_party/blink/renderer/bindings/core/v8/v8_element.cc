...
static void toggleAttributeMethod(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ExceptionState exceptionState(info.GetIsolate(), ExceptionState::kExecutionContext, "Element", "toggleAttribute");
  ...
  if (UNLIKELY(numArgsPassed <= 1)) {
    bool result = impl->toggleAttribute(qualifiedName, exceptionState);
    if (exceptionState.HadException()) {
      return;
    }
  ...
  bool result = impl->toggleAttribute(qualifiedName, force, exceptionState);
  if (exceptionState.HadException()) {
    return;
  }
  V8SetReturnValueBool(info, result);
...
void V8Element::toggleAttributeMethodCallback(const v8::FunctionCallbackInfo<v8::Value>& info) {
  ...
  ElementV8Internal::toggleAttributeMethod(info);
}
